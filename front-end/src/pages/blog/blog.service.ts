...
export const blogApi = createApi({
  ...
  tagTypes: ['Posts'], //nhung kieu tag cho phep dung trong blogApi
  ...
  endpoints: (builder) => ({
    // generic types, theo thu tu la kieu du lieu respone sau do la kieu du lieu cua arg
    getPostList: builder.query<Post[], void>({
      query: () => 'posts', // day la method GET, ko co arg
      // configuration for an individual endpoint, overriding the api setting
      keepUnusedDataFor: 5,
      /**
       * providesTags có thể là array hoặc callback return array
       * Nếu có bất kỳ một invalidatesTag nào match với providesTags này
       * thì sẽ làm cho getPostList method chạy lại
       * và cập nhật lại danh sách các bài post cũng như các tags phía dưới
       */
      providesTags: (result) => {
        /**
         * Cái callback này sẽ chạy mỗi khi getPostList chạy
    ...
    addPost: builder.mutation<Post, Omit<Post, 'id'>>({
       ...
       * Trong trường hợp này getPostList sẽ chạy lại
       */
    ...
    updatePost: builder.mutation<Post, { id: string; body: Post }>({
      ...
      // Trong trường hợp này thì getPostList sẽ chạy lại
